...
export default function BundleEngine() {
  ...
  const calc = useMemo(() => {
    ...
    return { tier, customerDiscount, supplierDiscount, customerUnitPrice, feeUnit, savingsUnit, totalSavings, yourSavings, toNext };
  ...
  return (
    <section className={cn(CARD, "p-5 sm:p-7")}>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-semibold tracking-tight text-slate-900">Smart Bündel · Rabattstaffel</h2>
      ...
      <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50 p-5">
        ...
        {calc.toNext ? (
          <p className="mt-4 border-t border-slate-200 pt-3 text-sm text-slate-600">
            Noch <span className="font-semibold text-slate-900">{chf(calc.toNext.needed, 0)} {unit}</span> bis Tier {calc.toNext.tier} — dann {calc.toNext.disc}% Rabatt für alle Teilnehmer.
          </p>
        ) : (
          <p className="mt-4 border-t border-slate-200 pt-3 text-sm font-medium text-accent">
            Höchste Rabattstufe erreicht — Tier 3, 20% Rabatt.
          </p>
        )}
      </div>

      {/* Rabattstaffel — kompakte, sachliche Referenz */}
      <div className="mt-6 overflow-hidden rounded-lg border border-slate-200">
        <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-2">
          <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Rabattstaffel</span>
          <span className="inline-flex items-center gap-1.5 text-[11px] text-slate-400">
            <ShieldCheck className="h-3.5 w-3.5 text-accent" /> Garantiert für alle Teilnehmer
          </span>
        </div>
        <table className="w-full text-left text-[13px]">
          <tbody className="divide-y divide-slate-100">
            {TIERS.map((t) => {
              const active = t.tier === calc.tier.tier;
              const range = t.max === null ? `ab ${t.min} ${unit}` : `${t.min}–${t.max} ${unit}`;
              return (
                <tr key={t.tier} className={cn(active && "bg-accent/[0.04]")}>
                  <td className="w-px whitespace-nowrap py-2.5 pl-4 pr-3">
                    <span className={cn("text-[11px] font-semibold uppercase tracking-wider", active ? "text-accent" : "text-slate-400")}>
                      Tier {t.tier}
                    </span>
                  </td>
                  <td className="py-2.5 pr-3 text-slate-600">{range}</td>
                  <td className="py-2.5 pr-4 text-right">
                    <span className={cn("text-base font-bold tabular-nums", active ? "text-accent" : "text-slate-700")}>
                      −{t.discount}%
                    </span>
                    {active && (
                      <span className="ml-2 align-middle text-[10px] font-bold uppercase tracking-wider text-accent">
                        · aktuell
                      </span>
                    )}
                  </td>
