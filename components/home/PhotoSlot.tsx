...
export default function PhotoSlot({
  ...
  return (
    <div
      className={`flex h-full w-full flex-col justify-between bg-slate-100 p-4 ${className}`}
      style={{
        backgroundImage:
    ...
    >
      ...
      <div>
        <p className="text-[12.5px] font-semibold leading-snug text-slate-600">{slot.alt}</p>
        <p className="mt-1.5 text-[11px] leading-relaxed text-slate-400">Suche: {slot.search}</p>
      </div>
    </div>
  );
}
