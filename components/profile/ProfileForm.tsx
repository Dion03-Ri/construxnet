...
function Section({
  ...
  return (
    /* Kein Kasten um jeden Abschnitt. Eine Haarlinie oben, die
       Überschrift, fertig — dieselbe Machart wie im Feed und in der
       Bündelliste. Vier umrandete Blöcke untereinander waren genau die
       Kästchenwand, die weg sollte. */
    <section className="border-t border-slate-200 pt-7 first-of-type:border-t-0 first-of-type:pt-0">
    ...
    <form action={formAction} className="space-y-7">
      ...
      <Section title="Firma" icon={Building2} hint="So erscheint dein Betrieb im Netzwerk, im Feed und auf der Karte.">
        <div className="flex flex-wrap items-center gap-4">
          <span className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-lg bg-slate-100 text-base font-bold text-slate-700 ring-1 ring-white/10">
            {logoUrl ? (
      ...
      <div className="sticky bottom-20 z-10 flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white/95 px-4 py-3 shadow-card backdrop-blur md:bottom-4">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-md bg-brand px-5 py-2.5 text-sm font-semibold text-navy-950 transition-colors hover:bg-brand-600 disabled:opacity-60"
        >
