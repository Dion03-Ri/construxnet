      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {ROLES.map((r) => (
            <label
              key={r.key}
              className="group relative cursor-pointer rounded-lg border border-white/[0.08] bg-[#0B1522] p-4 transition-colors hover:border-brand/40 has-[:checked]:border-brand has-[:checked]:bg-brand/10"
            >
              <input
                type="radio"
                name="role"
              ...
              />
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/10 text-white/70 peer-checked:bg-brand/20 peer-checked:text-brand">
      ...
      {isSupplier && (
        <div className="space-y-4 rounded-lg border border-brand/25 bg-brand/[0.04] p-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-white">
            ...
            <span className="text-[11px] font-normal text-white/55">— damit Smart Bündeln weiss, ob du für einen Auftrag infrage kommst</span>
          </div>

          <div>
            <span className={labelClass}>Welche Materialien lieferst du? *</span>
            <div className="max-h-72 space-y-3 overflow-y-auto rounded-md border border-white/[0.08] bg-[#0B1522] p-3">
              {PROC_CATEGORIES.map((cat) => {
                const items = PROC_MATERIALS.filter((m) => m.category === cat);
                if (items.length === 0) return null;
                return (
                  <div key={cat}>
                    <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-white/40">{cat}</div>
                    <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
                      {items.map((m) => (
                        <label key={m.key} className="flex cursor-pointer items-center gap-2 rounded-md border border-white/[0.08] bg-[#0B1522] px-3 py-2 text-[13px] text-white/75 transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand/10">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {PROC_REGIONS.map((r) => (
                <label key={r} className="cursor-pointer rounded-md border border-white/[0.08] bg-[#0B1522] px-3 py-1.5 text-[13px] text-white/75 transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand/10 has-[:checked]:text-brand">
