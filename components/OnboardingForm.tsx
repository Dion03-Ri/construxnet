  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {ROLES.map((r) => (
            <label
              key={r.key}
              className="group relative cursor-pointer rounded-lg border border-slate-200 bg-white p-4 transition-colors hover:border-brand/50 has-[:checked]:border-brand has-[:checked]:bg-brand-50"
            >
              <input
                type="radio"
                name="role"
        ...
        <div className="space-y-4 rounded-lg border border-brand/30 bg-brand-50/60 p-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {PROC_REGIONS.map((r) => (
                <label key={r} className="cursor-pointer rounded-md border border-slate-200 bg-white px-3 py-1.5 text-[13px] text-slate-600 transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand-50 has-[:checked]:text-brand-700">
      ...
      <button
        type="submit"
        disabled={pending}
        className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-brand px-6 py-3 text-sm font-semibold text-navy-950 transition-colors hover:bg-brand-500 disabled:cursor-not-allowed disabled:opacity-50"
      >
