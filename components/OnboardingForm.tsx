...
const inputClass =
  "w-full rounded-md border border-white/[0.16] bg-white/[0.03] px-3 py-2.5 text-sm text-white placeholder:text-white/[0.56] focus:border-brand focus:bg-[#16181a] focus:outline-none focus:ring-1 focus:ring-brand/30";
const labelClass =
  "mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/[0.72]";
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {ROLES.map((r) => (
            <label
              key={r.key}
              className="group relative cursor-pointer rounded-lg border border-white/[0.12] bg-[#16181a] p-4 transition-colors hover:border-brand/40 has-[:checked]:border-brand has-[:checked]:bg-brand/10"
            >
              <input
                type="radio"
                name="role"
              ...
              />
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/10 text-white/[0.72] peer-checked:bg-brand/20 peer-checked:text-brand">
                <r.icon className="h-5 w-5" />
              </span>
              <div className="mt-2 text-sm font-semibold text-white">{r.label}</div>
              <div className="text-xs text-white/[0.72]">{r.hint}</div>
            </label>
          ))}
        </div>
      </div>
      ...
      {isSupplier && (
        <div className="space-y-4 rounded-lg border border-brand/25 bg-brand/[0.04] p-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-white">
            ...
            <span className="text-[11px] font-normal text-white/[0.72]">— damit Smart Bündeln weiss, ob du für einen Auftrag infrage kommst</span>
          </div>

          <div>
            <span className={labelClass}>Welche Materialien lieferst du? *</span>
            <div className="max-h-72 space-y-3 overflow-y-auto rounded-md border border-white/[0.12] bg-[#16181a] p-3">
              {PROC_CATEGORIES.map((cat) => {
                const items = PROC_MATERIALS.filter((m) => m.category === cat);
                if (items.length === 0) return null;
                return (
                  <div key={cat}>
                    <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-white/[0.56]">{cat}</div>
                    <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
                      {items.map((m) => (
                        <label key={m.key} className="flex cursor-pointer items-center gap-2 rounded-md border border-white/[0.12] bg-[#16181a] px-3 py-2 text-[13px] text-white/[0.72] transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand/10">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {PROC_REGIONS.map((r) => (
                <label key={r} className="cursor-pointer rounded-md border border-white/[0.12] bg-[#16181a] px-3 py-1.5 text-[13px] text-white/[0.72] transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand/10 has-[:checked]:text-brand">
