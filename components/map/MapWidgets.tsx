...
function Panel({
  ...
  return (
    <div className={"border-t border-white/[0.12] pt-5"}>
      ...
      {hint && <p className="mt-0.5 text-[12px] text-white/[0.72]">{hint}</p>}
...
function BarList({
  ...
  if (items.length === 0) {
    return <p className="py-3 text-[12.5px] leading-relaxed text-white/[0.56]">{emptyText}</p>;
  }
      ...
      {items.map((i) => (
        <li key={i.label}>
          <div className="mb-1 flex items-center justify-between gap-2 text-[13px]">
            <span className="truncate text-white/[0.72]">{i.label}</span>
...
export default function MapWidgets() {
  ...
  return (
    <div className="space-y-4">
      <div className={"border-t border-white/[0.12] py-4"}>
        <div className="grid grid-cols-2 divide-x divide-white/[0.12]">
          <div className="pr-4">
            <div className="text-[11px] font-medium uppercase tracking-wider text-white/[0.56]">
              Baustoffwerke
            </div>
        ...
        <p className="mt-2.5 flex items-start gap-1.5 text-[11.5px] leading-relaxed text-white/[0.56]">
      ...
      <Panel title="Liefergebiet" icon={Truck}>
        {radius ? (
          <div className="text-[13px] text-white/[0.72]">
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-bold text-white">{radius.median}</span>
              <span className="text-white/[0.72]">km im Median</span>
            </div>
            <p className="mt-1.5 text-[12.5px] text-white/[0.72]">
              Spannweite {radius.min}–{radius.max} km, aus {radius.n} Profil
              {radius.n === 1 ? "" : "en"} mit Angabe.
            </p>
          </div>
        ) : (
          <p className="py-1 text-[12.5px] leading-relaxed text-white/[0.56]">
