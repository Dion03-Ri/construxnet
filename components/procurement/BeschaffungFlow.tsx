...
export default function BeschaffungFlow({
  ...
  /**
   * Vorlaeufige Referenznummer, nur zur Anzeige.
   *
   * Stand hier frueher in einem useMemo mit Math.random(). Das laeuft auf
   * dem Server und im Browser mit verschiedenen Werten — React hat die
   * Seite deshalb bei jedem Aufruf mit einem Hydration-Fehler neu
   * gezeichnet. Jetzt entsteht sie erst nach dem Einhaengen.
   */
  const [reference, setReference] = useState("");
  useEffect(() => {
    setReference("OBT-" + new Date().getFullYear() + "-" + Math.floor(1000 + Math.random() * 8999));
  }, []);
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-8 text-center">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          ...
          <Link href="/dashboard" className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600 transition-colors hover:bg-slate-100">
            Zum Dashboard
          </Link>
          <button type="button" onClick={reset} className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600 transition-colors hover:bg-slate-100">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 sm:p-6">
            ...
            <motion.div key={step} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.18 }}>
                  ...
                  <div className="mt-3 grid max-h-[360px] grid-cols-1 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">
                    {filteredMaterials.map((m) => {
                      ...
                      return (
                        ...
                        >
                          <span className={cn("mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded border", active ? "border-brand bg-brand text-navy-950" : "border-slate-300 bg-white")}>
              ...
              {step === 2 && (
                <div>
                  ...
                  >
                    <span className={cn("mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-md border", pool ? "border-brand bg-brand text-navy-950" : "border-slate-300 bg-white")}>
          ...
          <div className="mt-6 flex items-center justify-between border-t border-white/[0.06] pt-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button type="button" onClick={next} disabled={!canNext || submitting} className="inline-flex items-center gap-1.5 rounded-md bg-brand px-5 py-2 text-sm font-semibold text-navy-950 transition-colors hover:bg-brand/100 disabled:cursor-not-allowed disabled:opacity-50">
                {step === STEPS.length - 1 ? (
                  submitting ? (
...
function CustomMaterialModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-accent-600/60 backdrop-blur-sm" onClick={onClose} />
      <motion.div
      ...
      >
        ...
        <div className="space-y-3.5 px-5 py-4">
          ...
          >
            <span className={cn("mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded border", share ? "border-brand bg-brand text-navy-950" : "border-slate-300 bg-white")}>
