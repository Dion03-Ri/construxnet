...
export default function CompanyConnect({ targetId }: { targetId: string }) {
  ...
  if (myId === targetId) {
    return (
      <span className="inline-flex items-center rounded-lg border border-white/[0.12] bg-[#16181a] px-4 py-2 text-sm text-white/[0.72]">
        Das ist dein Profil
      </span>
    );
  }
  ...
  const messageBtn = (
    <Link
      ...
      className="inline-flex items-center gap-1.5 rounded-lg border border-white/[0.12] bg-[#16181a] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-white/[0.07]"
    >
  ...
  if (conn?.status === "CONNECTED") {
    return (
      <div className="flex flex-wrap gap-2">
        <span className="inline-flex items-center gap-1.5 rounded-lg bg-accent/15 px-4 py-2 text-sm font-medium text-brand">
  ...
  if (conn?.status === "PENDING" && conn.direction === "outgoing") {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-lg border border-white/[0.12] bg-[#16181a] px-4 py-2 text-sm font-medium text-white/[0.72]">
